...
async function getHomeData() {
  try {
    ...
    const [
      { data: collections }, { data: newArtworks }, { data: featuredArtists },
      { count: artistCount }, { count: artworkCount },
    ] = await Promise.all([
    ...
    return { collections: collections??[], newArtworks: newArtworks??[], featuredArtists: featuredArtists??[], artistCount: artistCount??0, artworkCount: artworkCount??0 }
  } catch {
...
export default async function HomePage({ params: { locale } }: Props) {
  ...
  return (
    <>
      ...
      <FeaturedCollections collections={d.collections} title={t('featured_collections')} viewAll={t('view_all')} />
      <NewWorks artworks={d.newArtworks} title={t('new_works')} viewAll={t('view_all')} />
      <FeaturedArtists artists={d.featuredArtists} title={t('featured_artists')} viewAll={t('view_all')} />
      <AboutSection artistCount={d.artistCount} artworkCount={d.artworkCount} locale={locale} />
    </>
  )
}
